...
const Play = () => {
  ...
  const getLegalStacks = () => {
    return gameState.stacks
      .map((_, index) => ({ index, legal: getStackLegality(index) }))
      .filter(s => s.legal);
  };
  ...
  return (
    <div className={cn(
      ...
      gameEffects.shakeContainer && "container-shake",
      gameEffects.showRedFlash && "red-flash"
    )}>
      {/* Confetti overlay */}
      {gameEffects.showConfetti && (
      ...
      <section className="tsw-hero">
        <div className="tsw-hero__inner">
          <div className="tsw-hero__title">
            <h1>Stack your cards like Taylor stacks her eras.</h1>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto">
          {gameState.stacks.map((stack, index) => (
            <StackColumn
              ...
              dimmed={focusedElement !== null && focusedElement !== index}
              onHover={handleStackHover}
              shouldHighlight={shouldHighlightStack(index)}
            />
          ))}
        </div>
